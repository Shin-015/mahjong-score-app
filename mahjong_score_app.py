    ...
    with engine.begin() as conn:
        result = conn.execute(
            text("""
                ...
                RETURNING id
            """),
            {"session_id": int(session_id), "hanchan_no": int(hanchan_no)},
        )
...
def update_hanchan_result(session_id, hanchan_no, final_scores):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("DELETE FROM hanchan_results WHERE hanchan_id = :hanchan_id"), {"hanchan_id": int(hanchan_id)})
